    ...
    execute: function () {
      ...
      _export("MahjongTile", MahjongTile = (_dec = ccclass('MahjongTile'), _dec(_class = class MahjongTile extends Component {
        ...
        onLoad() {
          ...
          var slotsNode = find('Canvas/Slots');
        ...
        onMouseDown(event) {
          ...
          if (uiTransform) {
            var localMousePos = uiTransform.convertToNodeSpaceAR(new Vec3(event.getUILocation().x, event.getUILocation().y, 0));
        ...
        onMouseUp(event) {
          ...
          var tileCenter = this.node.getComponent(UITransform).convertToWorldSpaceAR(Vec3.ZERO); // 检查是否在格子内

          var closestGridNode = null;
          var minDistance = Infinity;

          for (var i = 0; i < this.gridNodes.length; i++) {
            var gridNode = this.gridNodes[i];
            var gridTransform = gridNode.getComponent(UITransform);
            var gridCenter = gridTransform.convertToWorldSpaceAR(Vec3.ZERO); // 判断麻将牌的中心点是否与格子的边界框相交

            if (gridTransform.getBoundingBoxToWorld().contains(tileCenter)) {
              // 计算麻将牌中心与格子中心的距离
              var distance = tileCenter.subtract(gridCenter).length();
          ...
          if (closestGridNode) {
            var existingTile = this.gridNodeMap.get(closestGridNode);

            if (existingTile) {
              // 如果格子中已有麻将，与当前麻将交换位置
              var existingTilePosition = existingTile.getWorldPosition();
...
//# sourceMappingURL=b7e340e7c1515d0cf28c1bfa8e44ce0cb3f5107b.js.map